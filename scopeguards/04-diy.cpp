class Meta {
public:
    explicit Meta(MYSQL_RES* meta) : meta_(meta) {}

    ~Meta() {
        mysql_free_result(meta);
    }

    operator MYSQL_RES*() const { return meta_; }

private:
    MYSQL_RES* const meta_;
};

void Bind::operator+=(const db::Query& query) {

    const auto meta = Meta(mysql_stmt_result_metadata(stmt_));
    if (!meta) {
